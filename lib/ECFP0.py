...
#                    if (self.atom[i].ele > 18 and self.atom[j].ele > 18):
    ...
    def GenSubStr(self, atomlist):
        ...
        #for atom in self.atom:
        #    if atom.ele> 18:
        #        substr.atom.append(atom)
        for i in atomlist:
            substr.atom.append(self.atom[i])

        try:
            substr.label = self.label
        except:
            substr.label = ''
        substr.abc = self.abc
        substr.energy = 0.0
        ...
        #substr.bmx2D = np.array([[0 for i in range(len(atomlist))] for j in range(len(atomlist))])
        substr.bmx2D = np.zeros((len(atomlist),len(atomlist)),dtype=np.int)
        for i,reali in enumerate(atomlist):
            for j,realj in enumerate(atomlist):
                substr.bmx2D[i][j]= self.bmx2D[reali][realj]
        substr.surfaceatom = [0 for i in range(substr.natom)]
        return substr
...
class AllStr(allstr):
    ...
    def GetAllFragECFPname(self, numproc = 24, flag =2, savefp = False):
        #if flag == 1:
        ...
        for Str in self:
            ...
            Str.AllFragECFPname = []
            for frag in substr:
                frag.ECFPname = (frag.GenECFPname(savefp = savefp))[0]
                #print frag.ECFPname
