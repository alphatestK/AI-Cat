...
class Str(object):
    ...
    def CalMass(self):
        allmass = np.array([PT.Elemass[atom.ele-1] for atom in self.atom])
        self.mass = allmass.sum()
        return allmass.sum()
    ...
    def screenuppersurf(self,):
        self.upper =0
        for atom in self.atom:
            if (atom.ele < 18) and (atom.xyz[2] > 0.9*self.abc[2]):
                self.upper =1
                break
            if (atom.ele < 18) and (atom.xyz[2] < 0.1*self.abc[2]):
                self.upper =1
                break

    def sortatombyele(self,Lreverse =False):
        self.atom.sort(key =lambda X: X.ele, reverse = Lreverse)
    ...
    #def voigt_6_to_full_3x3_stress(self):
    def to_full_3x3_stress(self):
        """
        Form a 3x3 stress matrix from a 6 component vector in Voigt notation
        """
        #s1, s2, s3, s4, s5, s6 = self.stress[:6]
        #return np.transpose([[s1, s6, s5],
        #                 [s6, s2, s4],
        #                 [s5, s4, s3]])


        self.stress_33 =  np.transpose([[self.stress[0], self.stress[1], self.stress[2]],
                             [self.stress[1], self.stress[3], self.stress[4]],
                             [ self.stress[2], self.stress[4], self.stress[5]]])
    ...
    def outPOSCAR(self,outfile, fixatom = False):
        f=open(outfile,'w')
        f.write('system\n')
        ...
        for iatom,atom in enumerate(self.atom):
            if (not fixatom):
                ...
                if ((iatom+1 ) < fixatom[0]) or ((iatom+1 ) > fixatom[1]):
                    f.write('%12.8f  %12.8f  %12.8f T T T\n'%(atom.xyz[0],atom.xyz[1],atom.xyz[2]))
                else:
                    f.write('%12.8f  %12.8f  %12.8f F F F\n'%(atom.xyz[0],atom.xyz[1],atom.xyz[2]))
        f.close()
...
#        return dis
    ...
    def shortestbond(self,ele1,ele2,lim):
        ...
        if (len(minlist) >0) :short = min(minlist)
        return short
    ...
    def allbondbyele(self,ele1,ele2 , flag =1):
        ...
        for iatom in range(self.natom):
            if self.atom[iatom].ele != ele1: continue
        ...
        for i in range(self.natom):
            if (i != iatom):
                ...
                if bondorder != 0 :#and self.atom[i].ele == 8:
                    ...
                    if self.atom[i].ele == elelist[5]:
                        self.atom[iatom].bondtype.append(bondorder+100000)
                    if self.atom[i].ele == elelist[4]:
                        self.atom[iatom].bondtype.append(bondorder+10000)
    ...
    def Get_chemformula(self):
        self.chemform = ""
        for i,ele in enumerate(self.elenameList):
            self.chemform = self.chemform + "%s%d"%(ele,self.natompe[i])

    def Get_AllFragFormula(self):
        self.chemformula = ""
        substr = [[] for i in np.unique(self.group)]
        for id,atom in enumerate(self.atom):
            #if atom.ele > 18:
            #    continue
            atom.id = id
            substr[self.group[id]-1].append(atom.elesymbol)
        substr = sorted(substr, key=lambda x:len(x), reverse=True)

        self.allfragchemformula = []
        for _substr in substr:
            _string = "%s" %reduce(lambda a,b: a+b, [ "%s%d" %(x,_substr.count(x)) for x in set(_substr)])
            self.chemformula = self.chemformula + _string+ " "
            self.allfragchemformula.append(_string)
...
#        return bool(Lminstr.contents)
    ...
    def CheckMin(self,flag = 1):
        ...
        self.calCtypes()
        program='./Lib_fillbond/checkminbond.so'
    ...
    def Bondmatrix (self, ):
        program='./Lib_bondmatrix/bondmatrix.so'
        self.calCtypes()
        sqnatm = self.natom**2
        self.cdnt2fcnt()
    ...
    def ExpandStr(self,expand =1.0):
        newstr= Str()

        self.cdnt2fcnt()
        self.abc[0] = self.abc[0]*expand
        self.abc[1] = self.abc[1]*expand
        self.abc[2] = self.abc[2]*expand
        self.abc2lat()
    ...
    def Translation(self, direction, dis):
        for atom in self.atom:
            atom.xyz[direction-1] = atom.xyz[direction-1]+ dis
...
#
    ...
    def myfilter(self,BadStrP):
        if(self.Energy > BadStrP.HighE or \
           self.Energy < BadStrP.LowE or \
...
#          max(self.Latt[:3]) > 5.0*min(self.Latt[:3]) or \
          ...
          #return True
           if len(self.For) :
              ...
              if self.maxF > BadStrP.MaxFor : return False
              else: return True
           else:
              return True
