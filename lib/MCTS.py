...
#        return self.rpdict[Str.ECFPname]
    ...
    def Predict(self,Str,selectsurface = 0, LaddZPE = 0):
        if len(self) == 1:
            self.pop(0)
        self.append(Str)
    ...
    def Predict_fixedsurface(self,Str,selectsurface, LaddZPE= 0):
        if len(self) == 1:
            self.pop(0)
        self.append(Str)
        self.PredictProduct_fixedsurface(selectsurface, LaddZPE= LaddZPE)
...
#            path.GetBarrier_list()
    ...
    def PathReproduce(self,filename, LGibbs= 0, selectsurface =False, LZPE = 0):
        f = open('result','w')
        test = AllPair()
