            ...
            for j in xrange(i+1,self.natom):
                if self.bmx2D[i][j]>0:
                    if self.atom[i].ele > 18 and self.atom[j].ele > 18:
                        self.atom[i].nbmetal.append(j)
                        self.atom[j].nbmetal.append(i)
...
#                        self.atom[j].nb.append([self.bmx2D[j][i],i,self.supercellinfo[j,i][1]])
    ...
    def ScreenUpperSurfaceAtom(self,):
        self.UpperSurface = 0
        for iatom,atom in enumerate(self.atom):
            atom.UpperSurface =0
            if (atom.ele < 18) : continue
            if len(atom.nbmetal) >= 11: continue
            if atom.xyz[-1] < (self.atom[-1].xyz[-1]/2+0.5):  continue
            atom.UpperSurface =1
            self.UpperSurface =  self.UpperSurface +1
            #self.UpperSurface.append(iatom)
        #print self.UpperSurface
        if (self.UpperSurface != 9) and (self.UpperSurface != 16) and (self.UpperSurface != 24) :
            print ('Warning:  Possible Wrong Surface Extract')
        return
...
class singlefp(object):
    ...
    def genidentifier(self,dim=10000000):
        ...
        self.allatom.sort()
        _allatomset = np.unique(self.allatom)
        _pureid = []
        for id_periodic in _allatomset:
...
#                        self.cyclenb[(iatom,ilayer+1)].append([subatom.bond[inb],newatom.id])
    ...
    def InitIdentifier(self):
        ...
        for iatom,atom in enumerate(self.str.atom):
            ...
            del newfp.allatomset
            if (iatom in self.str.addmetal) or (atom.ele< 18):# or (atom.UpperSurface == 1):
    ...
    def UpdateLayer(self,layer):
        ...
        for ifp,fp in enumerate(newfplist):
            if fp.save ==1:
                ...
                if (ifp in self.str.addmetal) or (self.str.atom[ifp].ele< 18 ):#  or ( self.str.atom[ifp].UpperSurface == 1):
