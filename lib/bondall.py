                ...
                if nowbond[-1]!= 1 :
                    if orderneed == 0:
                        if temp1[-1] == 1:
                            ...
                            temp2[-1]= temp2[-1]-1
                            nowbond[-1]=nowbond[-1]-1
                            return self.reduce_bondcombine(nowbond,numneed),self.reduce_bondcombine(temp2,numneed)


                    elif orderneed !=0 :
                        if temp1[-1] == 1:
                            nowbond[-1]=nowbond[-1]-1
                            return self.reduce_bondcombine(temp1,numneed),self.reduce_bondcombine(nowbond,numneed)
                        else:
                            temp2= temp1[:]
                            temp2[-1]= temp2[-1]-1
                            nowbond[-1]=nowbond[-1]-1
                            return self.reduce_bondcombine(temp1,numneed),self.reduce_bondcombine(nowbond,numneed)#,self.reduce_bondcombine(temp2,numneed)
            ...
            if bond.num == tempin[-1].num:
                ...
                if len(tempin) ==nbond: #for nbond ==1
                    ...
                    if ( i< ibond-1 ):
                        ...
                        return  self.fillbondtype(order,nbond,tempin,templist)
                ...
                if len(tempfill) == nbond and i< ibond-1:
                    ...
                    tempin[-1] = templist[i+1]
                    return self.fillbondtype(order,nbond,tempfill2,templist), self.fillbondtype(order,nbond,tempin,templist)
                elif len(tempfill) == nbond and i ==(ibond-1):
                    self.orderfill.append([x.num for x in tempfill])
                    return
                elif len(tempfill)< nbond and i <(ibond-1):
                    tempin[-1] = templist[i+1]
                    return self.fillbondtype(order,nbond,tempin,templist),self.fillbondtype(order,nbond,tempfill,templist)
                elif len(tempfill)<nbond and i ==(ibond -1):
